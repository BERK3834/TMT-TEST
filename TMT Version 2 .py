...
class TrailMakingTest:
    ...
    def start_screen(self):
        ...
        tk.Label(self.root, text="Trail Making Test Kuralları", font=("Arial", 16, "bold"), fg="white", bg="black").pack(pady=20)
        ...
        rules = ("Bu test, bilişsel yeteneklerinizi ve görsel-motor hızınızı ölçmek için tasarlanmıştır.\n"
                 ...
                 "- Test süresi 5 dakikadır.\n"
                 "- 'Deneme' butonuyla alıştırma yapabilir, 'Başla' butonuyla testi başlatabilirsiniz.")
        ...
        tk.Button(self.root, text="Deneme", font=("Arial", 14), bg="gray", fg="white", command=lambda: self.start_test(is_trial=True)).pack(pady=10)
        tk.Button(self.root, text="Başla", font=("Arial", 14, "bold"), bg="gray", fg="white", command=lambda: self.start_test(is_trial=False)).pack(pady=10)    
    
    def start_test(self, is_trial=False):
        ...
        self.correct_sequence = self.sequence_A if is_trial else self.sequence_B
        ...
        for value in self.current_sequence:
            ...
            btn = tk.Button(self.root, text=value, font=("Arial", 12), bg="gray", fg="white", width=5, height=2,
                            command=lambda b=value: self.check_selection(b))
            btn.place(x=x, y=y)  
    ...
    def get_non_overlapping_position(self):
        """Butonları rastgele yerleştirirken üst üste gelmelerini engeller."""
        max_attempts = 100  
    ...
    def finish_test(self):
        ...
        result_text = f"Test Tamamlandı!\nGeçen Süre: {elapsed_time} saniye\nDoğru: {self.correct_answers}, Yanlış: {self.wrong_answers}\nSonuç: {result}"
